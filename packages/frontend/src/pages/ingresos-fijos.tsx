              ...
              {ingresosFijos?.map((inf) => (
                <TableRow key={inf.id}>
                  ...
                  <TableCell className="text-right font-mono">
                    {inf.monto_mensual != null ? formatMonto(inf.monto_mensual, moneda) : "—"}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {inf.monto_anual != null ? formatMonto(inf.monto_anual, moneda) : "—"}
                  </TableCell>
...
function NuevoIngresoFijoDialog({
  open,
  onOpenChange,
  entidades,
}: {
      ...
      <DialogContent>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate() }} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <Label htmlFor="if-anual">Monto Anual</Label>
              <Input
                id="if-anual"
                type="number"
                step="0.01"
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="if-mensual">Monto Mensual</Label>
              <Input
                id="if-mensual"
                type="number"
                step="0.01"
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="if-ent">Entidad</Label>
            <Select value={entidadId} onValueChange={setEntidadId}>
              <SelectTrigger id="if-ent"><SelectValue placeholder="Opcional" /></SelectTrigger>
              <SelectContent>
                <SelectGroup>
