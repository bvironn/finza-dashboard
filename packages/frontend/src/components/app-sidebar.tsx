    ...
    <Sidebar collapsible="icon">
      ...
      <SidebarFooter>
        {collapsed ? (
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton tooltip={`${mes ? MESES_NOMBRE[mes - 1] + " " : ""}${anio}`}>
                <CalendarIcon />
                <span>{anio}</span>
              </SidebarMenuButton>
            </SidebarMenuItem>
            ...
            <div className="flex items-center rounded-md border">
              ...
              >
                <div className="flex flex-col min-w-0">
                  <span className="text-xs font-medium truncate">{user?.nombre}</span>
                  <span className="text-[10px] text-muted-foreground">{user?.rol}</span>
                </div>
              </button>
              <Button type="button" variant="ghost" size="icon-sm" className="shrink-0 mr-1" onClick={logout}>
                <LogOutIcon />
              </Button>
            </div>
...
function AccountDialog({
  ...
  const updateMutation = useMutation({
    mutationFn: () =>
      api.updateMe({ nombre, email }),
    onSuccess: () => {
      // Update local auth state
      if (user) login(localStorage.getItem("finza_token")!, { ...user, nombre, email })
