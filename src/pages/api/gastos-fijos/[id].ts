...
export const PUT: APIRoute = async ({ request, params }) => {
  ...
  await db
    .update(gastos_fijos)
    .set(updates)
    .where(and(eq(gastos_fijos.id, id), eq(gastos_fijos.user_id, auth.user.parentUserId)))
...
export const DELETE: APIRoute = async ({ request, params }) => {
  ...
  await db.delete(gastos_fijos).where(and(eq(gastos_fijos.id, id), eq(gastos_fijos.user_id, auth.user.parentUserId)))
  return json({ ok: true })
}
