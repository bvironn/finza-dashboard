...
export const PUT: APIRoute = async ({ request, params }) => {
  ...
  try {
    await db
      ...
      .where(and(eq(subcategorias.id, id), eq(subcategorias.user_id, auth.user.parentUserId)))
...
export const DELETE: APIRoute = async ({ request, params }) => {
  ...
  const rows = await db.execute(
    sql`SELECT COUNT(*) as cnt FROM transacciones WHERE subcategoria_id = ${id} AND user_id = ${auth.user.parentUserId}`
  )
  ...
  await db.delete(subcategorias).where(and(eq(subcategorias.id, id), eq(subcategorias.user_id, auth.user.parentUserId)))
  return json({ ok: true })
}
