...
import type { DesgloseDimension } from "@/types"
  ...
  return (
    <div className="flex flex-col gap-6">
      <h2 className="text-2xl font-semibold">
        Dashboard — {mes ? `${MESES[mes - 1]} ${anio}` : anio}
      </h2>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <KPICard title="Total Ingresos" value={totales?.ingresos} moneda={moneda}
          icon={<TrendingUpIcon className="text-emerald-600" />} loading={loadingTotales}
          className="border-emerald-200 dark:border-emerald-900" />
        <KPICard title="Total Gastos" value={totales?.gastos} moneda={moneda}
          icon={<TrendingDownIcon className="text-red-600" />} loading={loadingTotales}
          className="border-red-200 dark:border-red-900" />
        <KPICard title="Balance Neto" value={totales?.balance} moneda={moneda}
          icon={<WalletIcon />} loading={loadingTotales}
      ...
      <Card>
        ...
        <CardContent>
          {loadingResumen ? <Skeleton className="h-[400px]" /> : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {filteredResumen?.map((m) => (
                  <TableRow key={m.mes} className={`cursor-pointer hover:bg-muted/50 ${m.proyectado ? "opacity-60 italic" : ""}`}
                    onClick={() => window.location.href = `/transacciones?mes=${m.mes}`}>
                    <TableCell className="font-medium">
                      {m.nombre}
                      {m.proyectado && <span className="ml-1.5 text-xs font-normal text-muted-foreground">(proy.)</span>}
                    </TableCell>
                    <TableCell className="text-right">{formatMonto(m.ingresos, moneda)}</TableCell>
                    <TableCell className="text-right">{formatMonto(m.gastos, moneda)}</TableCell>
                    <TableCell className={`text-right font-medium ${balanceColor(m.balance)} ${balanceBg(m.balance)} rounded`}>
                      {formatMonto(m.balance, moneda)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      <Card>
        ...
        <CardContent>
          <Tabs defaultValue="categorias">
            ...
            <TabsContent value="categorias"><DesgloseTable data={desgloseCat} moneda={moneda} /></TabsContent>
            <TabsContent value="subcategorias"><DesgloseTable data={desgloseSub} moneda={moneda} /></TabsContent>
            <TabsContent value="entidades"><DesgloseTable data={desgloseEnt} moneda={moneda} /></TabsContent>
          </Tabs>
        </CardContent>
      </Card>
    </div>
...
function KPICard({ title, value, moneda, icon, loading, className = "", valueClass = "" }: {
  title: string; value?: number; moneda: string; icon: React.ReactNode
  loading: boolean; className?: string; valueClass?: string
}) {
  return (
