...
import type { Transaccion, TransaccionInput } from "@/lib/types"
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-semibold">
          Transacciones — {mes ? `${MESES_NOMBRE[mes]} ${anio}` : anio}
        </h2>
              ...
              <TableBody>
                {data?.data.map((tx) => (
                  <TableRow key={tx.id}>
                    ...
                    <TableCell>
                      <Badge variant={tx.tipo === "INGRESO" ? "default" : "destructive"}>
                        {tx.tipo}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right font-mono">
                      {formatMonto(tx.monto, moneda)}
                    </TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex flex-col gap-1 text-xs">
                      <span className="text-emerald-700 dark:text-emerald-400">
                        +{formatMonto(totalsFiltered.ingresos, moneda)}
                      </span>
                      <span className="text-red-700 dark:text-red-400">
                        -{formatMonto(totalsFiltered.gastos, moneda)}
                      </span>
...
function TransaccionDialog({
  ...
  // Populate form when dialog opens or transaccion changes
  useEffect(() => {
    if (open && transaccion) {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              ...
              <Select value={categoriaId} onValueChange={setCategoriaId}>
                <SelectTrigger id="tx-cat"><SelectValue placeholder="Seleccionar" /></SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    {categorias.map((c) => (
                      <SelectItem key={c.id} value={String(c.id)}>{c.nombre}</SelectItem>
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="tx-sub">Subcategoría</Label>
              <Select value={subcategoriaId} onValueChange={setSubcategoriaId}>
                <SelectTrigger id="tx-sub"><SelectValue placeholder="Seleccionar" /></SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    {subcategorias.map((s) => (
                      <SelectItem key={s.id} value={String(s.id)}>{s.nombre}</SelectItem>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <Label htmlFor="tx-ent">Entidad</Label>
              <Select value={entidadId} onValueChange={setEntidadId}>
                <SelectTrigger id="tx-ent"><SelectValue placeholder="Seleccionar" /></SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    {entidades.map((e) => (
                      <SelectItem key={e.id} value={String(e.id)}>{e.nombre}</SelectItem>
            ...
            <div className="flex flex-col gap-2">
              ...
              <Select value={tipo} onValueChange={(v) => setTipo(v as "GASTO" | "INGRESO")}>
                <SelectTrigger id="tx-tipo"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="GASTO">Gasto</SelectItem>
                    <SelectItem value="INGRESO">Ingreso</SelectItem>
