...
import { transacciones, categorias, subcategorias, entidades } from "../db/schema"
...
export async function getResumenMensualConProyeccion(anio: number, userId: number): Promise<ResumenMes[]> {
  ...
  // Get fixed expenses and income totals
  ...
  return base.map((mes) => {
    // For future months in the current or future year, project fixed amounts
