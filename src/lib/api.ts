...
export const api = {
  ...
  updateMe: (data: { nombre?: string; email?: string }) =>
    fetchJSON<{ ok: boolean }>("/api/auth/me", { method: "PUT", body: JSON.stringify(data) }),
  ...
  createSubUser: (data: { email: string; password: string; nombre: string; permisos?: Partial<SubUserPermisos> }) =>
  ...
  createUser: (data: { email: string; password: string; nombre: string; rol: string; parent_id?: number; permisos?: Partial<SubUserPermisos> }) =>
