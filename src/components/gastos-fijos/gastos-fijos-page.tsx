...
export function GastosFijosPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["gastos-fijos"] }); toast.success("Gasto fijo eliminado") },
  })
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setDialogOpen(true)}><PlusIcon className="mr-1 size-4" />Nuevo gasto fijo</Button>
      </div>
      <Card>
            ...
            <TableBody>
              {gastosFijos?.map((gf) => (
                <TableRow key={gf.id}>
                  ...
                  <TableCell className="text-right font-mono">{gf.monto_mensual != null ? formatMonto(gf.monto_mensual, moneda) : "—"}</TableCell>
                  <TableCell className="text-right font-mono">{gf.monto_anual != null ? formatMonto(gf.monto_anual, moneda) : "—"}</TableCell>
                  <TableCell>{gf.entidad ?? "—"}</TableCell>
                  <TableCell><Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(gf.id)}><TrashIcon /></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
            <TableFooter>
      ...
      <NuevoGastoFijoDialog open={dialogOpen} onOpenChange={setDialogOpen} entidades={entidades ?? []} />
    </div>
  )
}

function NuevoGastoFijoDialog({ open, onOpenChange, entidades }: { open: boolean; onOpenChange: (v: boolean) => void; entidades: { id: number; nombre: string }[] }) {
  const queryClient = useQueryClient()
  ...
  const mutation = useMutation({
    mutationFn: () => api.createGastoFijo({
    ...
    onSuccess: () => {
      ...
      toast.success("Gasto fijo creado"); onOpenChange(false)
