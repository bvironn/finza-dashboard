...
const apiMap = {
  categorias: {
    ...
    create: api.createCategoria,
    update: api.updateCategoria,
    delete: api.deleteCategoria,
  },
  subcategorias: {
    get: api.getSubcategorias,
    create: api.createSubcategoria,
    update: api.updateSubcategoria,
    delete: api.deleteSubcategoria,
  },
  entidades: {
    get: api.getEntidades,
    create: api.createEntidad,
    update: api.updateEntidad,
    delete: api.deleteEntidad,
  },
}
    ...
    <div className="flex flex-col gap-6">
      ...
      <Tabs defaultValue="categorias">
        ...
        <TabsContent value="categorias">
          <CatalogoSection type="categorias" title="Categorías" />
        </TabsContent>
        <TabsContent value="subcategorias">
          <CatalogoSection type="subcategorias" title="Subcategorías" />
        </TabsContent>
        <TabsContent value="entidades">
          <CatalogoSection type="entidades" title="Entidades" />
        </TabsContent>
      </Tabs>
    </div>
  )
                  ...
                  {editingId === item.id ? (
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        size="icon"
                        variant="ghost"
                        onClick={() => updateMutation.mutate({ id: item.id, nombre: editingName })}
                      >
                        <CheckIcon />
                      </Button>
                  ...
                  {editingId !== item.id && (
                    <div className="flex gap-1">
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => { setEditingId(item.id); setEditingName(item.nombre) }}
                      >
                        <PencilIcon />
                      </Button>
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => { setDeleteInfo(item); setDeleteError(null) }}
                      >
                        <TrashIcon />
                      </Button>
                    </div>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {deleteError
                  ? deleteError
                  : "Se eliminará permanentemente. Si hay transacciones asociadas no se podrá eliminar."}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancelar</AlertDialogCancel>
              {!deleteError && (
                <AlertDialogAction onClick={() => deleteInfo && deleteMutation.mutate(deleteInfo.id)}>
                  Eliminar
                </AlertDialogAction>
              )}
            </AlertDialogFooter>
...
function CreateDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Agregar a {title.toLowerCase()}</DialogTitle>
        </DialogHeader>
        <form
          onSubmit={(e) => {
            e.preventDefault()
