...
export const PUT: APIRoute = async ({ request, params }) => {
  ...
  try {
    await db
      ...
      .where(and(eq(categorias.id, id), eq(categorias.user_id, auth.user.parentUserId)))
...
export const DELETE: APIRoute = async ({ request, params }) => {
  ...
  await db.delete(categorias).where(and(eq(categorias.id, id), eq(categorias.user_id, auth.user.parentUserId)))
  return json({ ok: true })
}
