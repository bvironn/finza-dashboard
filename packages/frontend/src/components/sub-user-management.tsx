...
import type { Usuario, SubUserPermisos } from "@/lib/types"
...
export function SubUserManagement() {
  ...
  const { data: limit } = useQuery({
    queryKey: ["sub-users", "limit"],
    queryFn: api.getSubUserLimit,
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Sub-usuario eliminado")
      setDeleteId(null)
    },
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle>Sub-usuarios</CardTitle>
          <CardDescription>
            {limit ? `${limit.current} de ${limit.max} disponibles` : "Cargando..."}
          </CardDescription>
        </div>
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Eliminar sub-usuario?</AlertDialogTitle>
...
function PermisosForm({
  ...
  permisos: SubUserPermisos
  onChange: (p: SubUserPermisos) => void
}) {
  return (
        ...
        {(Object.keys(PERMISOS_LABELS) as (keyof SubUserPermisos)[]).map((key) => (
          <div key={key} className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(v: boolean) => onChange({ ...permisos, [key]: v })}
            />
          </div>
        ))}
      </div>
...
function CreateSubUserDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["sub-users"] })
      toast.success("Sub-usuario creado")
      onOpenChange(false)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Nuevo sub-usuario</DialogTitle>
        </DialogHeader>
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate() }} className="flex flex-col gap-4">
          ...
          <Button type="submit" disabled={mutation.isPending}>
            {mutation.isPending ? "Creando..." : "Crear sub-usuario"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
...
function EditSubUserDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["sub-users"] })
      toast.success("Sub-usuario actualizado")
      onClose()
    },
    onError: (err: Error) => toast.error(err.message),
  })
  ...
  return (
    <Dialog open onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar sub-usuario</DialogTitle>
        </DialogHeader>
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate() }} className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <Label>Nombre</Label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-2">
              <Label>Nueva contraseña (vacío = sin cambios)</Label>
